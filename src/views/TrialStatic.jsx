...
export default function TrialStatic({expLen=20, step,nxtUrl,attentionChecks=[[],[]],randomTrickQ=[[],[]] ,randomNormalQ = [[],[]]}) {
    ...
    const firstQIndex =  isFirstItemCheck()?getQuestion(attentionChecks[1][0]): (isFirstItemNormal()? getQuestion(randomNormalQ[[1][0]]):randomTrickQ[1][0])
    ...
    const trimStaticTrials = (curIdx,data) => {
        ...
        randomTrickQ = reConstructRandomArray(randomTrickQ,data,curIdx)
        console.log(randomTrickQ,"trick after trim")
        console.log(randomNormalQ,"normal after trim")
    ...
    const nextIndex = async () => {
       ...
       if(randomNormalQ[0].includes(idx+1)){
           ...
           await addRecord(userID, idx, firebaseNewRecord, expName).then(async () => {
               ...
               setAnswer('')
               const nextQID = randomTrickQ[1][0]
               setQidx(nextQID);
               setActiveQuestion(getQuestion(nextQID))
               setIdx(idx + 1);
               if (activeQuestion.img === curImg)
